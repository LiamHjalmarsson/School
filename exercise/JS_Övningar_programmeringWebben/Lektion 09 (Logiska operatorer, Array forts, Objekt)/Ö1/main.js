// Placera era utskrifter efter denna arrayen
let users = [
    {
        "gender": "female",
        "name": {
...
// 1. Skriv ut det användarnamn (username) Dale Bailey använder vid login
console.log(users[2].login.username)

console.log(users.filter(user => user.name.first == "Dale").map(user => user.login.username)[0]);
...
console.log(users.find(user => user.name.first == "Dale").login.username);

// 2. Skriv ut mobilnumret (cell) från den användare vars email är "kuzey.basoglu@example.com"
function email (email) {
    return users.find(user => user.email == email).cell
}

// console.log(email("kuzey.basoglu@example.com"))

// 3. Skriv ut både gatunamn och nummer från Terry Medina
function address (first, last) {
    return users.filter(user => user.name.first == first && user.name.last == last).map(user => user.location.street.name + " " + user.location.street.number)[0]
}
// console.log(address("Terry", "Medina"))

// 4. Skriv ut titel, förnamn och efternamn från den person som bor i Danmark
function country (country) {
    return users.filter(user => user.location.country == country).map(user => `${user.name.title} ${user.name.first} ${user.name.last}`)[0]
}

// console.log(country("Denmark"))

// 5. Skriv ut beskrivningen över den tidzon som Miss Jimenez bor i
function timeZone (name) {
    return users.find(user => user.name.last == name).location.timezone.description
}

console.log(timeZone("Jimenez"))